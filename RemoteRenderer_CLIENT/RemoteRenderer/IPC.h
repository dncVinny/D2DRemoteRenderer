...
public:
    ...
    template<class C>
    void Send(C cRequest, C &cResponse)
    {
        void *pSharedBuffer = m_sMemory.Get<void>();
        memcpy(pSharedBuffer, &cRequest, sizeof(C));
        ...
        memcpy(&cResponse, pSharedBuffer, sizeof(C));
    }
    template<class C>
    void Send(C cRequest)
    {
        memcpy(m_sMemory.Get<void>(), &cRequest, sizeof(C));
